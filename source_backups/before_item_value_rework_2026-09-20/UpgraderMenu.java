...
public class UpgraderMenu extends AbstractContainerMenu {
    ...
    private static int getItemValue(Item item) {
        ...
        if (stack.is(Items.DIAMOND_BLOCK) || stack.is(Items.EMERALD_BLOCK)) {
            return stack.is(Items.DIAMOND_BLOCK) ? 2_250 : 1_620;
        }
        if (stack.is(Items.GOLD_BLOCK)) {
            return 225;
        }
        if (stack.is(Items.RAW_GOLD_BLOCK)) {
            return 225;
        }
        if (stack.is(Items.IRON_BLOCK) || stack.is(Items.COPPER_BLOCK)) {
            return stack.is(Items.IRON_BLOCK) ? 180 : 72;
        }
        if (stack.is(Items.RAW_IRON_BLOCK) || stack.is(Items.RAW_COPPER_BLOCK)) {
            return stack.is(Items.RAW_IRON_BLOCK) ? 180 : 72;
        }
        if (stack.is(Items.REDSTONE_BLOCK) || stack.is(Items.LAPIS_BLOCK)) {
            return stack.is(Items.REDSTONE_BLOCK) ? 27 : 45;
        }
        ...
        if (stack.is(Items.RAW_GOLD)) return 25;
        if (stack.is(Items.RAW_IRON)) return 20;
        if (stack.is(Items.RAW_COPPER)) return 8;
        if (stack.is(Items.LAPIS_LAZULI) || stack.is(Items.QUARTZ) || stack.is(Items.AMETHYST_SHARD)) {
            return stack.is(Items.AMETHYST_SHARD) ? 15 : 5;
        }
        if (stack.is(Items.REDSTONE) || stack.is(Items.COAL) || stack.is(Items.CHARCOAL)) {
            return stack.is(Items.REDSTONE) ? 3 : 4;
        }

        String itemId = BuiltInRegistries.ITEM.getKey(item).getPath();
        if (itemId.equals("enchanted_golden_apple")) return 1_200;
        if (itemId.equals("golden_apple")) return 220;
        if (itemId.equals("elytra")) return 2_000;
        if (itemId.equals("dragon_egg")) return 5_000;
        if (itemId.equals("nether_star")) return 1_000;
        if (itemId.equals("beacon")) return 1_200;
        if (itemId.equals("totem_of_undying")) return 600;
        if (itemId.equals("trident")) return 800;
        if (itemId.equals("mace")) return 1_800;
        if (itemId.equals("heavy_core")) return 1_000;
        if (itemId.equals("heart_of_the_sea")) return 300;
        if (itemId.equals("shulker_shell")) return 100;
        if (itemId.equals("wither_skeleton_skull")) return 350;
        if (itemId.equals("echo_shard")) return 80;
        if (itemId.endsWith("diamond_ore")) return 250;
        if (itemId.endsWith("emerald_ore")) return 180;
        if (itemId.endsWith("gold_ore")) return 25;
        if (itemId.endsWith("iron_ore")) return 20;
        if (itemId.endsWith("copper_ore")) return 8;
        if (itemId.equals("enchanting_table")) return 500;
        if (itemId.equals("ender_chest")) return 300;
        if (itemId.equals("anvil") || itemId.equals("chipped_anvil") || itemId.equals("damaged_anvil")) return 200;
        if (itemId.equals("hopper") || itemId.equals("observer") || itemId.equals("piston")) return 24;
        if (itemId.startsWith("netherite_")) return 1_800;
        if (itemId.startsWith("diamond_")) return 300;
        if (itemId.startsWith("golden_") || itemId.startsWith("gold_")) return 35;
        if (itemId.startsWith("iron_")) return 28;
        if (itemId.equals("ancient_debris")) return 280;
        if (itemId.equals("ender_pearl") || itemId.equals("blaze_rod")) return 30;
        ...
        if (rarityValue > 1) return rarityValue;
        if (item.getDefaultInstance().getMaxStackSize() == 1) return 12;
        return 1;
    }
